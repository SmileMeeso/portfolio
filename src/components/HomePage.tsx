...
const coreCompetencies: {
  ...
  },
  {
    ...
    body: "테스트가 없거나, 있어도 믿을 수 없으면 결국 사람이 매번 확인하게 됩니다. 무작위로 실패하던 E2E의 원인을 규명해 신뢰할 수 있는 상태로 만들고, Storybook으로 모듈화와 문서화를 함께 확보했습니다. 제가 없어도 굴러가는 상태를 만드는 것까지가 일의 끝이라고 봅니다.",
    evidence:
      "omelet — 무작위 실패 2~8건에서 flaky 0건, E2E 37→99건 · SK 하이닉스 — 배포당 수동 QA 2~3일에서 1일 이내로 단축 · I.Log.Yo — 안정화 후 다른 FE 개발자에게 인수인계 완료",
    color: "#EA580C",
  },
];
    ...
    projects: [
      {
        ...
        problem: [
          ...
          "E2E가 라우트의 9%만 덮고 있었고 실행마다 2~8건이 무작위로 실패해, 통과해도 통과했다고 믿을 수 없는 상태였습니다.",
          "커버리지를 측정할 인프라 자체가 없어 어느 화면이 검증되지 않았는지 알 방법이 없었습니다.",
          "성능을 로컬 DevTools 측정에만 의존해, 실제 사용자 환경에서 어떤 화면이 느린지 알 수 없었습니다.",
          "프론트엔드 취약점 이슈가 반복적으로 발생해 보안 뉴스를 빠르게 확인할 수 있는 체계가 필요했습니다.",
        ],
        impact: [
          "풀필먼트 솔루션의 첫 고객사 납품을 성공적으로 이끌었습니다.",
          "실행마다 2~8건씩 무작위로 실패하던 E2E를 flaky 0건으로 만들었습니다. 3회 연속 99건 전부 통과했고, 1.1~7.2분으로 들쭉날쭉하던 실행 시간도 1.6분으로 안정됐습니다.",
          "E2E를 37건에서 99건으로 늘려(+168%) 라우트 커버리지를 9.4%에서 45.2%로, 앱 소스 라인 커버리지를 58.8%까지 끌어올렸습니다.",
          "실사용자 p75 기준 LCP 529ms · INP 41.7ms · CLS 0.04 로 Core Web Vitals 세 지표 모두 Google 권장 Good 구간을 유지하고 있습니다. 간헐적으로 튀는 구간(LCP 최대 7.5s, CLS 최대 0.84)도 화면·배포 단위로 추적할 수 있게 됐습니다.",
        ...
        contributions: [
          ...
          "무작위 실패의 원인을 refresh 토큰 회전과 storageState 스냅샷 충돌로 규명하고 제거해 E2E를 신뢰할 수 있는 상태로 전환",
          "전부를 덮기보다 인증·주문·운영관리·배송최적화 등 핵심 플로우를 우선순위로 잡고 E2E 99건(11개 spec)을 설계·구현",
          "커버리지 측정 인프라를 V8 기반으로 구축. 프레임워크 런타임과 외부 SDK가 분모에 섞여 부풀려진 71%를 걷어내고, 앱 소스만 필터링한 58.8%를 팀의 기준 지표로 정의",
          "실패하던 jest 3개 스위트 4건을 수정해 워크스페이스 유닛 테스트를 전부 통과 상태로 복구",
          "Datadog RUM 을 도입해 실사용자 기준 Core Web Vitals(LCP·INP·CLS)를 상시 수집하고, 화면별 지표를 대시보드로 팀에 공유",
